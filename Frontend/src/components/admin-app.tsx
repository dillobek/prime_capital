...
type Item=Record<string,unknown>&{id:string;title?:string;description?:string;name?:string;phone?:string;email?:string;username?:string;phpInvest?:number;primeCapital?:number;url?:string;views?:number;user?:{id?:string;name?:string;email?:string;phone?:string;username?:string;telegramId?:string}|null};
...
function PromotionReports(){
  const [list,setList]=useState<Item[]>([]); const [status,setStatus]=useState(''); const [preview,setPreview]=useState<string|null>(null); const [filter,setFilter]=useState<'all'|'pending'|'approved'|'rejected'>('pending');
  ...
  const visible=list.filter(item=>filter==='all'||item.status===filter);
  return <><section className="panel list-panel wide-form"><div className="panel-head"><div><h2>Aksiya investitsiyasi tasdiqlari</h2><p>User yuborgan summalar faqat “Tasdiqlash” bosilganda balanslarga qo‘shiladi.</p></div><small>{status}</small></div><div style={{display:'flex',gap:8,flexWrap:'wrap',margin:'18px 0'}}>{[['pending','Yangi'],['all','Barchasi'],['approved','Tasdiqlangan'],['rejected','Rad etilgan']].map(([value,label])=><button key={value} className={filter===value?'primary':''} onClick={()=>setFilter(value as typeof filter)}>{label} ({value==='all'?list.length:list.filter(item=>item.status===value).length})</button>)}</div>{visible.map(item=><article className="content-row" key={item.id}><div><strong>PHP Invest: {usd(Number(item.phpInvestAmount??0))} · Prime Capital: {usd(Number(item.primeCapitalAmount??0))}</strong><p><b>{item.user?.name||'Noma’lum user'}</b> · {item.user?.phone||'Telefon yo‘q'} · {item.user?.email||'Email yo‘q'}</p><p>Telegram: {item.user?.username?`@${item.user.username.replace(/^@/,'')}`:'username yo‘q'} · ID: {item.user?.telegramId||item.userId}</p><p>Izoh: {String(item.description)}</p><span className={`status ${item.status}`}>{item.status==='pending'?'Kutilmoqda':item.status==='approved'?'Tasdiqlandi':'Rad etildi'}</span>{Array.isArray(item.images)?<div style={{display:'flex',gap:8,marginTop:10}}>{(item.images as string[]).map((src,i)=><img key={i} src={src} alt={`Dalil ${i+1}`} onClick={()=>setPreview(src)} style={{width:72,height:72,objectFit:'cover',borderRadius:8,cursor:'zoom-in'}}/>)}</div>:null}</div>{item.status==='pending'?<div className="request-actions"><button onClick={()=>decide(item.id,'approved')}>Tasdiqlash va balansga qo‘shish</button><button onClick={()=>decide(item.id,'rejected')}>Rad etish</button></div>:null}</article>)}{!visible.length?<div className="empty-state">Bu filterda aksiya tasdig‘i yo‘q.</div>:null}</section>{preview?<ImagePreview src={preview} onClose={()=>setPreview(null)}/>:null}</>
}
