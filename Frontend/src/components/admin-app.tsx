...
type Section='dashboard'|'balances'|'properties'|'about'|'banners'|'videos'|'notifications'|'requests'|'promotionReports'|'support'|'users'|'settings';
type Item=Record<string,unknown>&{id:string;title?:string;description?:string;name?:string;phone?:string;username?:string;phpInvest?:number;primeCapital?:number;url?:string;views?:number;user?:{id?:string;name?:string;phone?:string;username?:string;telegramId?:string}|null};
type AdminUser={id:string;name:string;email:string};
const menu:[Section,string,typeof Gauge][]=[['dashboard','Dashboard',Gauge],['balances','Balanslar',WalletCards],['properties','Kvartiralar',Building2],['about','Biz haqimizda',FileText],['banners','Aktsiyalar',Image],['videos','Video darslar',BookOpen],['notifications','Bildirishnomalar',Bell],['requests','So‘rovlar',Banknote],['promotionReports','Investitsiya tasdiqlari',ShieldCheck],['support','Support',Headphones],['users','Foydalanuvchilar',UsersRound],['settings','Sozlamalar',Settings]];
...
function PromotionReports(){
  const [list,setList]=useState<Item[]>([]); const [status,setStatus]=useState(''); const [preview,setPreview]=useState<string|null>(null); const [filter,setFilter]=useState<'all'|'pending'|'approved'|'rejected'>('pending'); const [search,setSearch]=useState('');
  const load=()=>request('/promotion-reports').then((items:Item[])=>{setList([...items].sort((a,b)=>String(b.createdAt??'').localeCompare(String(a.createdAt??''))));}).catch(()=>setList([])); useEffect(()=>{load()},[]);
  async function decide(id:string,decision:'approved'|'rejected'){setStatus('Saqlanmoqda...'); try{await request(`/promotion-reports/${id}/status`,{method:'PATCH',body:JSON.stringify({status:decision})}); await load(); setStatus(decision==='approved'?'Balansga qo‘shildi':'Hisobot rad etildi')}catch{setStatus('Xatolik yuz berdi')}}
  const query=search.trim().toLowerCase(); const visible=list.filter(item=>{const matchesStatus=filter==='all'||item.status===filter; const name=String(item.user?.name??'').toLowerCase(); const phone=String(item.user?.phone??'').toLowerCase(); return matchesStatus&&(!query||name.includes(query)||phone.includes(query));});
  return <><section className="panel list-panel wide-form"><div className="panel-head"><div><h2>Investitsiya tasdiqlari</h2><p>User yuborgan summalar faqat “Tasdiqlash” bosilganda balanslarga qo‘shiladi.</p></div><small>{status}</small></div><div className="report-toolbar"><div className="report-filters">{[['pending','Yangi'],['all','Barchasi'],['approved','Tasdiqlangan'],['rejected','Rad etilgan']].map(([value,label])=><button key={value} className={filter===value?'primary':''} onClick={()=>setFilter(value as typeof filter)}>{label} ({value==='all'?list.length:list.filter(item=>item.status===value).length})</button>)}</div><input className="report-search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Name yoki phone bo‘yicha qidirish..."/></div><div className="table-wrap report-table-wrap"><table className="report-table"><thead><tr><th>№</th><th>Name</th><th>Phone</th><th>PHP Invest</th><th>Prime Capital</th><th>Photo</th><th>Tasdiqlash</th><th>Rad etish</th></tr></thead><tbody>{visible.map((item,index)=><tr key={item.id}><td>{index+1}</td><td><b>{item.user?.name||'Noma’lum user'}</b></td><td>{item.user?.phone||'Telefon yo‘q'}</td><td>{usd(Number(item.phpInvestAmount??0))}</td><td>{usd(Number(item.primeCapitalAmount??0))}</td><td><div className="report-photos">{Array.isArray(item.images)?(item.images as string[]).map((src,i)=><img key={i} src={src} alt={`Dalil ${i+1}`} onClick={()=>setPreview(src)}/>):<span>—</span>}</div></td><td>{item.status==='pending'?<button className="report-approve" onClick={()=>decide(item.id,'approved')}>Tasdiqlash</button>:<span className={`status ${item.status}`}>{item.status==='approved'?'Tasdiqlandi':'Rad etildi'}</span>}</td><td>{item.status==='pending'?<button className="report-reject" onClick={()=>decide(item.id,'rejected')}>Rad etish</button>:<span>—</span>}</td></tr>)}</tbody></table></div>{!visible.length?<div className="empty-state">Bu filterda investitsiya tasdig‘i yo‘q.</div>:null}</section>{preview?<ImagePreview src={preview} onClose={()=>setPreview(null)}/>:null}</>
}
function ImagePreview({src,onClose}:{src:string;onClose:()=>void}){const [scale,setScale]=useState(1); return <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:100,background:'rgba(5,15,35,.82)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}><div onClick={e=>e.stopPropagation()} style={{position:'relative',maxWidth:'95vw',maxHeight:'95vh',display:'flex',flexDirection:'column',alignItems:'center',gap:14}}><div style={{display:'flex',gap:8,alignItems:'center'}}><button onClick={()=>setScale(s=>Math.min(4,s+.25))}>+</button><button onClick={()=>setScale(s=>Math.max(.5,s-.25))}>−</button><button onClick={()=>setScale(1)}>100%</button><button onClick={onClose}>Yopish</button></div><div style={{maxWidth:'90vw',maxHeight:'82vh',overflow:'auto',background:'#fff',borderRadius:10,padding:8}}><img src={src} alt="Investitsiya isboti" style={{display:'block',maxWidth:'none',width:`${scale*Math.min(900,window.innerWidth*0.82)}px`,height:'auto'}}/></div></div></div>}
function Requests(){
