...
function PromotionReports(){
  const [list,setList]=useState<Item[]>([]); const [status,setStatus]=useState(''); const [preview,setPreview]=useState<string|null>(null);
  const load=()=>request('/promotion-reports').then(setList).catch(()=>setList([])); useEffect(()=>{load()},[]);
  ...
  return <><section className="panel list-panel wide-form"><div className="panel-head"><div><h2>Aksiya investitsiyasi tasdiqlari</h2><p>User yuborgan summalar faqat “Tasdiqlash” bosilganda balanslarga qo‘shiladi.</p></div><small>{status}</small></div>{list.map(item=><article className="content-row" key={item.id}><div><strong>PHP Invest: {usd(Number(item.phpInvestAmount??0))} · Prime Capital: {usd(Number(item.primeCapitalAmount??0))}</strong><p>User: {String(item.userId)} · {String(item.description)}</p><span className={`status ${item.status}`}>{item.status==='pending'?'Kutilmoqda':item.status==='approved'?'Tasdiqlandi':'Rad etildi'}</span>{Array.isArray(item.images)?<div style={{display:'flex',gap:8,marginTop:10}}>{(item.images as string[]).map((src,i)=><img key={i} src={src} alt={`Dalil ${i+1}`} onClick={()=>setPreview(src)} style={{width:72,height:72,objectFit:'cover',borderRadius:8,cursor:'zoom-in'}}/>)}</div>:null}</div>{item.status==='pending'?<div className="request-actions"><button onClick={()=>decide(item.id,'approved')}>Tasdiqlash va balansga qo‘shish</button><button onClick={()=>decide(item.id,'rejected')}>Rad etish</button></div>:null}</article>)}{!list.length?<div className="empty-state">Hozircha aksiya tasdig‘i yo‘q.</div>:null}</section>{preview?<ImagePreview src={preview} onClose={()=>setPreview(null)}/>:null}</>
}
