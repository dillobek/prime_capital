...
export class PlatformService {
  ...
  private promotionReports: RecordItem[] = [];
  ...
  private load() {
    ...
    try {
      ...
      for (const key of ['users','banners','videos','notifications','investments','withdrawals','finance','support','promotionReports','percentHistory'] as const) if (Array.isArray(data[key])) this[key] = data[key];
  ...
  private save() {
    ...
    writeFileSync(this.dataFile, JSON.stringify({ users:this.users,banners:this.banners,videos:this.videos,notifications:this.notifications,investments:this.investments,withdrawals:this.withdrawals,finance:this.finance,support:this.support,promotionReports:this.promotionReports,lastPercent:this.lastPercent,balanceOverrides:this.balanceOverrides,lastAppliedAt:this.lastAppliedAt,percentHistory:this.percentHistory,about:this.about }, null, 2));
  }
  ...
  listNotifications(userId?: string, includePrivate = false) { return this.notifications.filter((item) => !item.targetUserId || includePrivate || item.targetUserId === userId); }
  ...
  createPromotionReport(userId: string, dto: PromotionReportDto) { if (Number(dto.phpInvestAmount ?? 0) <= 0 && Number(dto.primeCapitalAmount ?? 0) <= 0) throw new ConflictException('Kamida bitta investitsiya summasini kiriting'); const item = { id: id(), ...dto, userId, status: 'pending', createdAt: now() }; this.promotionReports.unshift(item); this.save(); return item; }
  listPromotionReports() { return this.promotionReports.map((report) => { const user = this.users.find((entry) => entry.id === report.userId); return { ...report, user: user ? { id: user.id, name: user.name, phone: user.phone, username: user.username, telegramId: user.telegramId } : null }; }); }
  approvePromotionReport(itemId: string, status: 'approved' | 'rejected') { const item = this.promotionReports.find((entry) => entry.id === itemId); if (!item) throw new NotFoundException(); if (item.status !== 'pending') throw new ConflictException('Hisobot allaqachon ko‘rib chiqilgan'); const user = this.users.find((entry) => entry.id === item.userId); if (!user) throw new NotFoundException('User topilmadi'); item.status = status; if (status === 'approved') { user.phpInvest = Number(user.phpInvest ?? 0) + Number(item.phpInvestAmount ?? 0); user.primeCapital = Number(user.primeCapital ?? 0) + Number(item.primeCapitalAmount ?? 0); } this.notifications.unshift({ id: id(), title: status === 'approved' ? 'Investitsiya haqidagi xabar tasdiqlandi' : 'Investitsiya haqidagi xabar rad etildi', description: status === 'approved' ? `PHP Invest: $${Number(item.phpInvestAmount ?? 0)} · Prime Capital: $${Number(item.primeCapitalAmount ?? 0)}. Balansingiz yangilandi.` : 'Iltimos, isbot rasmlari va ma’lumotlarni tekshirib, qayta yuboring.', targetUserId: item.userId, status: 'active', views: 0, createdAt: now() }); this.save(); return item; }
}
