...
export class PlatformService {
  ...
  listPromotionReports() { return this.promotionReports.map((report) => { const user = this.users.find((entry) => entry.id === report.userId); return { ...report, user: user ? { id: user.id, name: user.name, email: user.email, phone: user.phone, username: user.username, telegramId: user.telegramId } : null }; }); }
  approvePromotionReport(itemId: string, status: 'approved' | 'rejected') { const item = this.promotionReports.find((entry) => entry.id === itemId); if (!item) throw new NotFoundException(); if (item.status !== 'pending') throw new ConflictException('Hisobot allaqachon ko‘rib chiqilgan'); item.status = status; if (status === 'approved') { const user = this.users.find((entry) => entry.id === item.userId); if (!user) throw new NotFoundException('User topilmadi'); user.phpInvest = Number(user.phpInvest ?? 0) + Number(item.phpInvestAmount ?? 0); user.primeCapital = Number(user.primeCapital ?? 0) + Number(item.primeCapitalAmount ?? 0); } this.save(); return item; }
}
