...
function PromotionModal({onClose}:{onClose:()=>void}){ const [status,setStatus]=useState(''); const [done,setDone]=useState(false); async function submit(e:FormEvent<HTMLFormElement>){ e.preventDefault(); const f=new FormData(e.currentTarget); const imageValue=f.get('images'); const files=imageValue instanceof FileList?Array.from(imageValue):[]; if(files.length<1||files.length>5){setStatus('1 dan 5 tagacha rasm tanlang');return;} const phpInvestAmount=Number(f.get('phpInvestAmount')||0), primeCapitalAmount=Number(f.get('primeCapitalAmount')||0); if(phpInvestAmount<=0&&primeCapitalAmount<=0){setStatus('PHP Invest yoki Prime Capital summasini kiriting');return;} setStatus('Yuborilmoqda…'); const images=await Promise.all(files.map(file=>new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result));reader.onerror=reject;reader.readAsDataURL(file)}))); try{await request('/promotion-reports',{method:'POST',body:JSON.stringify({phpInvestAmount,primeCapitalAmount,description:f.get('description'),images})});setDone(true);setStatus('Xabaringiz yuborildi. Admin tekshirganidan so‘ng balansingiz to‘ldiriladi.')}catch(err){setStatus(err instanceof Error?err.message:'Xatolik yuz berdi')} } return <div className="modal-overlay" onClick={onClose}><div className="modal-card" onClick={e=>e.stopPropagation()}><div className="modal-head"><h2>Aksiya haqida xabar berish</h2><button onClick={onClose}><X size={18}/></button></div>{done?<p className="modal-success">{status}</p>:<form onSubmit={submit}><p>Investitsiya summalarini kiriting va tasdiqlovchi rasmlarni yuboring.</p><label>PHP Invest summasi ($)<input name="phpInvestAmount" type="number" min="0" step="0.01" placeholder="0"/></label><label>Prime Capital summasi ($)<input name="primeCapitalAmount" type="number" min="0" step="0.01" placeholder="0"/></label><label>Izoh<textarea name="description" rows={3} required/></label><label>Rasmlar (1–5 ta)<input name="images" type="file" accept="image/*" multiple required/></label><button className="primary" type="submit">Yuborish</button>{status?<small>{status}</small>:null}</form>}</div></div> }
